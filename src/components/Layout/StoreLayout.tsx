...
export const StoreLayout = ({ children, requireAuth = false }: StoreLayoutProps) => {
  ...
  useEffect(() => {
    // Redirecione usuários admin para o painel administrativo quando acessarem páginas da loja
    if (!loading && user && userRole === 'admin' && location.pathname.startsWith('/store')) {
      navigate('/produtos', { replace: true });
      return;
    }
    
    // Redirecione usuários não autenticados para login se a página requer autenticação
